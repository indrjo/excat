...
\begin{proof}
  ...
  l'unica che può esserci. Quindi \(f \circ 0_A^B\) è la composizione della
...
\begin{proof}
  ...
  frecce \(f \circ p_A\) e \(g \circ p_B\) sono di questo tipo, ma non
  vanno bene per i nostri scopi. Invece la somma sì, perché il diagramma
  \[
    \begin{tikzcd}[row sep=large]
      & C \\
      A \ar["{i_A}", r, swap] \ar["f", ur, bend left] & A + B \ar["{f
        \circ p_A + g \circ p_B}" description, u] & B \ar["{i_B}", l]
      \ar["g", ul, bend right, swap]
    \end{tikzcd}
  \]
  commuta.
\end{proof}
