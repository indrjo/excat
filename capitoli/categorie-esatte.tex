...
\(p_B \circ i_A = 0_A^B\) e \(p_A \circ i_B = 0_B^A\). E quindi è lecito
...
\begin{proof}
  ...
  \begin{equation}
    \begin{tikzcd}[row sep=tiny]
      A \oplus A' \ar["{f \oplus \id_{A'}}", r] & B \oplus A' \ar["{f' \circ
        p_{A'}^{BA'}}",
      r] & C \\
      B \oplus A' \ar["{\id_B \oplus g}", r, swap] & B \oplus B' \ar["{g'
        \circ p_{B'}^{BB'}}", r, swap] & C'
    \end{tikzcd}
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Ricordando ora che \(p_{C}^{C0} : C \oplus 0 \to C\) è un
  isomorfismo e che
  \(p_{C}^{C0} \circ \left( g \oplus 0_{A'}^0 \right) = g \circ
  p_{B}^{BA'}\), abbiamo finito.
\end{proof}
